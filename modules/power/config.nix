...
let
  powerManagementPowertopEnable = if host.name == "zeus" then true else false;
  powerManagementCpuFreqGovernor = if host.name == "zeus" then "powersave" else "performance";
  bootKernelParams = if host.name == "zeus" then [
    "amd_pstate=active"
